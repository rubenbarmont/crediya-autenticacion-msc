...
import co.com.crediya.model.user.exceptions.InvalidCredentialsException; // <-- Crearemos esta excepción
...
public class LoginUseCase {
    ...
    public Mono<String> execute(String email, String rawPassword) {
        return userRepository.findByEmail(email)
                ...
                .flatMap(this::enrichUserWithRole) // <-- NUEVO PASO EN EL FLUJO
                .flatMap(tokenManager::generate);
    }

    private Mono<User> verifyPassword(User user, String rawPassword) {
        // Asumimos que el modelo User tiene un campo 'password'. Si no, debemos agregarlo.
        return passwordVerifier.verify(rawPassword, user.getPassword())
                .filter(Boolean::booleanValue) // Filtramos, solo continuamos si el resultado es 'true'
                .map(isValid -> user) // Si es válido, devolvemos el usuario
                .switchIfEmpty(Mono.error(new InvalidCredentialsException())); // Si el filtro no pasa, las credenciales son inválidas.
    }

    // --- NUEVO MÉTODO PRIVADO PARA BUSCAR Y AÑADIR EL ROL ---
    private Mono<User> enrichUserWithRole(User user) {
        // Si el idRol es nulo, es un error de integridad de datos.
        ...
        return rolRepository.findById(user.getIdRol())
                ...
                .map(rol -> {
                    user.setRol(rol); // Enriquece el objeto User con la información del Rol
                    return user;
                });
    }
